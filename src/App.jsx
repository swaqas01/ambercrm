...
const AP_PERIODS = [["today", "Today"], ["yesterday", "Yesterday"], ["week", "This Week"], ["lastweek", "Last Week"], ["month", "This Month"], ["lastmonth", "Last Month"], ["quarter", "Quarter"], ["year", "Year"]];
const AP_PERIOD_LABEL = { today: "today", yesterday: "yesterday", week: "this week", lastweek: "last week", month: "this month", lastmonth: "last month", quarter: "this quarter", year: "this year" };
function apRangeBounds(period, cf, ct) {
  const t = tgtTodayStr(); const Y = Number(t.slice(0, 4)); const M = Number(t.slice(5, 7));
  const mon = tgtMondayStr(); const p2 = (n) => String(n).padStart(2, "0");
  const mid = (y, m, d) => y + "-" + p2(m) + "-" + p2(d) + "T00:00:00+04:00";
  const sh = (dstr, days) => apShiftDate(dstr, days) + "T00:00:00+04:00";
  if (period === "today")     return { from: t + "T00:00:00+04:00", to: sh(t, 1) };
  if (period === "yesterday") return { from: sh(t, -1), to: t + "T00:00:00+04:00" };
  if (period === "week")      return { from: mon + "T00:00:00+04:00", to: sh(mon, 7) };
  if (period === "lastweek")  return { from: sh(mon, -7), to: mon + "T00:00:00+04:00" };
  if (period === "month")     return { from: mid(Y, M, 1), to: mid(M === 12 ? Y + 1 : Y, M === 12 ? 1 : M + 1, 1) };
  if (period === "lastmonth") { const ly = M === 1 ? Y - 1 : Y, lm = M === 1 ? 12 : M - 1; return { from: mid(ly, lm, 1), to: mid(Y, M, 1) }; }
  if (period === "quarter")   { const qs = M - ((M - 1) % 3); const qe = qs + 3; return { from: mid(Y, qs, 1), to: mid(qe > 12 ? Y + 1 : Y, qe > 12 ? qe - 12 : qe, 1) }; }
  if (period === "year")      return { from: mid(Y, 1, 1), to: mid(Y + 1, 1, 1) };
  const toIso = ct ? sh(ct, 1) : sh(t, 1);
  return { from: (cf || t.slice(0, 8) + "01") + "T00:00:00+04:00", to: toIso };
}
...
function AgentPerformance({ user, go, openAgent }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { from, to } = apRangeBounds(period, cf, ct);
        ...
        if (!alive) return;
        if (repR.error || (repR.data && repR.data.error)) throw new Error("Could not load performance.");
        const ags = (repR.data && repR.data.agents) || [];
        ...
        const now = Date.now(); const todayStr = tgtTodayStr();
        const tkey = (period === "today" || period === "yesterday") ? "today"
          : (period === "week" || period === "lastweek") ? "week"
          : (period === "month" || period === "lastmonth") ? "month" : null;
        const rows = ags.map((p) => {
          const myDevs = devByUser[p.id] || [];
          const activeNow = myDevs.some((dd) => !dd.revoked && dd.last_seen && (now - new Date(dd.last_seen)) < 30 * 60 * 1000);
          const lastTimes = [p.last_login, p.last_act].concat(myDevs.map((d) => d.last_seen)).filter(Boolean);
          const lastActive = lastTimes.length ? lastTimes.reduce((m, t) => (new Date(t) > new Date(m) ? t : m)) : null;
          const seenToday = lastTimes.some((t) => tgtDubaiDate(t) === todayStr);
          const tg = resolveTargets(defs, ovById[p.id]);
          const callTarget = tkey ? tg.call[tkey] : null;
          const waTarget = tkey ? tg.whatsapp[tkey] : null;
          const calls = p.calls, wa = p.whatsapps, reveals = p.reveals;
          const behind = callTarget != null && calls < callTarget;
          const warn = reveals >= 5 && (calls + wa) < reveals * 0.5;
          return { id: p.id, full_name: p.full_name, role: p.role, active: p.active, avatar_url: p.avatar_url,
            calls, wa, reveals, dealsClosed: p.deals_closed, netAmber: Number(p.commission || 0),
            leadsReceived: p.leads_received, leadsAssigned: p.leads_assigned, comments: p.comments,
            fuDone: p.fu_completed, fuPending: p.fu_pending, dealsSubmitted: p.deals_submitted,
            activeNow, activeDays: p.active_days, seenToday, lastActive, callTarget, waTarget, behind, warn };
        });
  ...
  let view = rows.filter((r) =>
    ...
    (statusF === "all"
      ...
      || (statusF === "warn" && r.warn)));
  const sortFns = {
    calls: (a, b) => b.calls - a.calls, wa: (a, b) => b.wa - a.wa, reveals: (a, b) => b.reveals - a.reveals, received: (a, b) => b.leadsReceived - a.leadsReceived,
  ...
  const T_behind = ["today", "yesterday", "week", "lastweek", "month", "lastmonth"].includes(period) ? rows.filter((r) => r.behind).length : null;
  ...
  return (
    <div style={{ maxWidth: 1180, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(138px,1fr))", gap: 11, marginBottom: 15 }}>
        ...
        <SumCard label={"Leads received " + plabel} value={tot("leadsReceived")} icon={<UserPlus size={13} />} />
      ...
      <div style={{ display: "flex", gap: 9, flexWrap: "wrap", marginBottom: 13, alignItems: "center" }}>
        ...
        {sel(sort, setSort, [["calls", "Sort: Calls"], ["wa", "Sort: WhatsApp"], ["reveals", "Sort: Reveals"], ["received", "Sort: Leads received"], ["deals", "Sort: Deals closed"], ...(canSeeCommission ? [["commission", "Sort: Commission"]] : []), ["behind", "Sort: Behind target"], ["active", "Sort: Last active"], ["low", "Sort: Lowest activity"]])}
            ...
            <div key={r.id} style={{ ...card, padding: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                ...
                <div style={{ flex: "2 1 300px", display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <Chip2 icon={<UserPlus size={13} />} value={r.leadsReceived} label="received" />
                  <Chip2 icon={<FileText size={13} />} value={r.comments} label="comments" />
                  <Chip2 icon={<CheckCircle2 size={13} />} value={r.fuDone} label={r.fuPending > 0 ? "fu done \u00b7 " + r.fuPending + " due" : "fu done"} color={T.ok} />
